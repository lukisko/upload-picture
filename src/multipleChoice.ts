...
export type MultipleChoiceProp = {
	numberOfOptions: number;//to delete
	correctAnswer: number;
	options: string[];
	//questions: string;
	padding: number;//to delete
	columns: number;//to delete
	height: number;//to delete
	width: number;//to delete
}
...
const database: QuestionInfo[] = require("../public/Questionare.json");
...
export default class MultipleChoice {
	...
	private async createTableOfQuestions(position: MRE.Vector3Like, questions: QuestionInfo[]) {
	...
	private createAnswers(parent: MRE.Actor, info: QuestionInfo) {
		...
		for (let i = 0; i < info.options.length; i++) {
			...
			optionButton.onClick((user) => {
				...
				if (i === info.correctOption - 1) {
					...
					const correctAnswer = parent.findChildrenByName(
						info.correctOption.toString(), false
					);
	...
	private updateQuestion(oldQuestion: MRE.Actor, info: QuestionInfo) {
	...
	private deleteQuestion(oldQuestion: MRE.Actor){
	...
	private addQuestion(info: QuestionInfo){
		database.push(info);
		let dataToWrite = JSON.stringify(database,null,2);
		//console.log(dataToWrite);
		fs.writeFile("./public/Questionare.json" ,dataToWrite , ()=>{});
	...
	private markItCorrect(parent: MRE.Actor) {
		parent.enableText({
			color: { r: 0, g: 1, b: 0 },
	...
	private markItIncorrect(parent: MRE.Actor) {
		parent.enableText({
			color: { r: 1, g: 0, b: 0 },
			contents: parent.text.contents,
			height: height,
